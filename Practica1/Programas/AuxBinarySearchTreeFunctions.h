...
void Insert(arbol_bin *BinaryTree, int newNumber)
{
	...
	while (*aux != NULL)
	{																		// Recorremos el árbol hasta encontrar el espacio libre donde irá el nuevo elemento
		if (newNumber > ((*aux)->number)) // En caso de que el valor sea mayor, iremos a la parte derecha del árbol
		{
			aux = &((*aux)->right);
		}
		else
		{ // Caso contrario, viajaremos a la parte izquierda del árbol
			aux = &((*aux)->left);
		}
	}
...
void InorderTraversal(arbol_bin *BinaryTree, int *Data, int size)
{
	...
	do
	{ // Iniciaremos el viaje por el árbol hasta que ya no haya apuntadores por los cuales viajar
		while (auxPos != NULL)
		{															// Haremos un recorrido hasta llegar a la parte más izquierda a partir de cierto nodo
			Stack[++stackTop] = auxPos; // Iremos colocando en la pila los nodos de la izquierda que vayamos encontrando
			auxPos = auxPos->left;
		}

		if (stackTop >= 0)
		{															// Una vez llegado a la parte más izquierda, verificamos si quedan nodos en la pila
